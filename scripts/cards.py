...
projects.json when set, otherwise the repo's own GitHub description; a `title`
there overrides the name drawn on the card without changing the filename.
...
def render_repo(repo, theme):
    ...
    # Shrink the title rather than let it run off the card - repo names here
    # include course-project slugs long enough to overflow twice over. Below
    # 10.5px it stops being readable, so past that it truncates instead.
    name = repo["name"]
    avail = W - pad - (pad + 22)
    size = 14.5
    while size > 10.5 and text_width(name, size) > avail:
        size -= 0.5
    if text_width(name, size) > avail:
        while len(name) > 1 and text_width(name + "\u2026", size) > avail:
            name = name[:-1]
        name = name.rstrip() + "\u2026"
    out.append(
        f'<text x="{pad + 22}" y="{pad + 12}" font-size="{size:g}" font-weight="700" '
...
def main(argv=None):
    ...
    for entry in wanted:
        ...
        if not src:
            ...
            continue
        card = {
            # `title` renames the card only; the file is still card-<repo>-*.svg
