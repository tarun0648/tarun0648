...
def from_github(user: str, token: str | None, limit: int, exclude: set[str],
    ...
    while True:
        ...
        for repo in repos:
            ...
            try:
                ...
                # Usually the 60/hour unauthenticated rate limit. Skipping the
                # repo silently would draw a confident chart off half the data,
                # so count these and warn once at the end.
    ...
    if skipped:
        print(f"  WARNING: {len(skipped)} repo(s) unreadable, so this chart is "
              f"drawn from partial data - set GITHUB_TOKEN and re-run "
